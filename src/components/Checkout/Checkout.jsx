...
const Checkout = () => {
  ...
  const handleQuantityChange = (id, newQty) => {
    setProducts(products.map(p => p.id === id ? { ...p, quantity: parseInt(newQty) } : p));
  };
    ...
    <div className="flex flex-col md:flex-row py-16 max-w-6xl w-full px-6 mx-auto">
      ...
      <div className="flex-1 max-w-4xl">
        <h1 className="text-3xl font-medium mb-6">
          Shopping Cart <span className="text-lg text-[#93B45D]">{products.length} Items</span>
        </h1>
        ...
        {products.map((product) => (
          <div key={product.id} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
            ...
            <button className="cursor-pointer mx-auto" onClick={() => handleRemove(product.id)}>
        ...
        <button className="group cursor-pointer flex items-center mt-8 gap-2 text-[#F18372] font-medium"
            onClick={()=>onclick(navigate("/products"))}
        >
      ...
      <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
          ...
          <div className="relative flex justify-between items-start mt-2">
            ...
            {showAddress && (
              <div className="absolute top-12 py-1 bg-white border border-gray-300 text-sm w-full z-10">
                <p onClick={() => { setSelectedAddress("New York, USA"); setShowAddress(false); }}
                   className="text-gray-500 p-2 hover:bg-gray-100 cursor-pointer">
                  New York, USA
                </p>
                <p onClick={() => { setSelectedAddress("Custom Address Added"); setShowAddress(false); }}
        ...
        <button
          ...
          className="w-full py-3 mt-6 cursor-pointer bg-[#F18372] text-white font-medium hover:bg-[#ec543d] transition"
        >
          Place Order
        </button>
      </div>
